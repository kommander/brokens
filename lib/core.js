 ...
 * A _Broken_ extends the native _Error_ Class, can be thrown and behaves just like expected.
 * With a sweet syntax.
 *
 ...
 * @api public
 */
function Broken(verb, object, reason, fnc) {
  if (!(this instanceof Broken)) {
    return new Broken(verb, object, reason, true);
  }
  ...
  Error.call(this);
  const obj = {};
  Error.captureStackTrace(obj, Broken);
...
Broken.prototype.__defineGetter__('stack', function message() {
  return `${this.message}\n${this._stack}`;
});
